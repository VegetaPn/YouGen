...
大家好，我有一个梦想——成为一名合格的梗王 🎭。

为了实现这个宏伟目标，我曾经每天花大量时间在推特上找有梗的帖子，并且练习写有梗的评论。但问题来了：这种方式效率实在太低，而且自己硬写评论也很难提升水平，我始终无法成为一名真正的梗王。

于是我想，为什么不做个工具来帮我呢？就这样，**有梗 YouGen** 诞生了——一个AI驱动的助手，它能：
- 自动发现热门推文
- 使用 Claude Opus 4.5 生成有梗的评论
- 通过审核和优化 AI 生成的内容来帮助我学习
- 让我专注于创意部分，而把繁琐的工作交给 AI

现已完成初版，并且**开源**。无论你是像我一样想成为梗王，还是只是想更高效地增长推特影响力，都欢迎使用！
...
编辑 `yougen/config/influencers.yaml`:
...
编辑 `yougen/config/user_profile.yaml` 来定制你的评论风格。
...
详细说明请查看 [SETUP_TWITTER_REAL_CHROME.md](doc/SETUP_TWITTER_REAL_CHROME.md)
...
参考 [SETUP_TWITTER_LOGIN.md](doc/SETUP_TWITTER_LOGIN.md)（可能遇到"浏览器不安全"错误）
